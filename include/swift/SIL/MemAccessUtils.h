...
inline bool isGuaranteedReturn(SILValue value) {
  auto *defInst = dyn_cast<ApplyInst>(value->getDefiningInstruction());
  if (!defInst) {
    return false;
  }
...
Result AccessUseDefChainVisitor<Impl, Result>::visit(SILValue sourceAddr) {
  ...
  case ValueKind::ApplyInst: {
    ...
    // TODO: Is a new access kind needed?
    if (isGuaranteedReturn(sourceAddr)) {
