...
bool swift::visitGuaranteedForwardingPhisForSSAValue(
  ...
  for (unsigned i = 0; i < guaranteedForwardingOps.size(); i++) {
    // TODO: We are assuming the GuaranteedForwarding results are @guaranteed
    // and can further forward @guaranteed values. This assumption exists in
    // other places in SILOWnershipVerifier as well. We should add verification
    // for this.
  ...
  for (unsigned i = 0; i < guaranteedForwardingOps.size(); i++) {
    for (auto val : guaranteedForwardingOps[i]->getUser()->getResults()) {
      for (auto *valUse : val->getUses()) {
        ...
        // If the visitor returns false, early exit.
        if (!visitor(valUse)) {
          return false;
        }
      }
...
void swift::visitExtendedGuaranteedForwardingPhiBaseValuePairs(
  ...
  // - Find phi value and new base value
  // - Call the visitor on the phi value and new base value pair
  // - Populate the worklist with pairs of guaranteed forwarding phi ops of phi
