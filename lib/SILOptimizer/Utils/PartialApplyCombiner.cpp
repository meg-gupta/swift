...
bool PartialApplyCombiner::copyArgsToTemporaries(
  ...
  getStoreBorrowArgsOfPartialApplyForExtension(pai, paiUsers,
                                               storeBorrowsToHandle);
  ...
  for (auto *storeBorrow : storeBorrowsToHandle) {
    if (canExtendStoreBorrowToNewUsers(storeBorrow, paiUsers, &deBlocks)) {
      extendStoreBorrowToNewUsers(storeBorrow, paiUsers, &deBlocks, callbacks);
      continue;
    }
    SILBuilderWithScope builder(pai, builderCtxt);
    // Copy address-arguments into a stack-allocated temporary.
