...
void GenericCloner::visitUncheckedTakeEnumDataAddrInst(
    UncheckedTakeEnumDataAddrInst *inst) {

  SILCloner<GenericCloner>::visitUncheckedTakeEnumDataAddrInst(inst);
  auto *clonedInst = cast<UncheckedTakeEnumDataAddrInst>(getOpValue(inst));
  if (inst->getFunction()->hasOwnership()) {
    if (!inst->getType().isTrivial(inst->getFunction()) &&
        clonedInst->getType().isTrivial(clonedInst->getFunction())) {
      enumInstsToFixup.push_back(clonedInst);
    }
  }
}
...
void GenericCloner::postFixUp(SILFunction *f) {
  ...
  public:
    AddressWalker(SSAPrunedLiveness &liveness) : liveness(liveness) {}
    bool visitUse(Operand *use) {
      liveness.updateForUse(use->getUser(), false);
      return true;
    }
  };

  for (auto *enumInst : enumInstsToFixup) {
    if (enumInst->use_empty()) {
      continue;
    }
    llvm::dbgs() << "derived from:\n";
    f->dump();
    SILBuilderWithScope::insertAfter(enumInst, [&](SILBuilder &builder) {
      SmallVector<SILBasicBlock *, 4> discoveredBlocks;
      SSAPrunedLiveness liveness(getCloned(), &discoveredBlocks);
      AddressWalker addressWalker(liveness);

      liveness.initializeDef(enumInst);
      auto result = addressWalker.walk(enumInst);
      auto isDynamic = (result == AddressUseKind::PointerEscape ||
                        result == AddressUseKind::Unknown);
                        
      auto loc = enumInst->getLoc();
      AllocStackInst *temp = nullptr;
      if (!isDynamic) {
        temp = builder.createAllocStack(
            loc, enumInst->getType(), std::nullopt,
            isDynamic ? HasDynamicLifetime_t::HasDynamicLifetime
                      : HasDynamicLifetime_t::DoesNotHaveDynamicLifetime);
      } else {
        assert(0);
      }

      enumInst->replaceAllUsesWith(temp);
      builder.createCopyAddr(loc, enumInst, temp, IsTake_t::IsTake,
                             IsInitialization_t::IsInitialization);
      builder.createDestroyAddr(loc, enumInst->getOperand());
      ...
      if (!isDynamic) {
        ...
        boundary.visitInsertionPoints([&](SILBasicBlock::iterator insertPt) {
          ...
          builder.createDeallocStack(insertPt->getLoc(), temp);
        });
      }
    });
  }

  if (!enumInstsToFixup.empty()) {
    StackNesting::fixNesting(getCloned());
  }
}
