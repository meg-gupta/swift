  ...
  for (auto *enumInst : enumInstsToFixup) {
    if (enumInst->use_empty()) {
      SILBuilderWithScope::insertAfter(enumInst, [&](SILBuilder &builder) {
        builder.createDestroyAddr(enumInst->getLoc(), enumInst->getOperand());
      });
      continue;
    }
    SILBuilderWithScope::insertAfter(enumInst, [&](SILBuilder &builder) {
      AddressWalker addressWalker;
      auto result = addressWalker.walk(enumInst);
      auto loc = enumInst->getLoc();
      auto *temp = builder.createAllocStack(loc, enumInst->getType());
      ...
      computeDominatedBoundaryBlocks(temp->getParent(), domInfo, boundary);
      for (SILBasicBlock *deallocBlock : boundary) {
        SILBuilderWithScope(deallocBlock->back().getIterator())
            .createDeallocStack(RegularLocation::getAutoGeneratedLocation(), temp);
      }
    });
  }
