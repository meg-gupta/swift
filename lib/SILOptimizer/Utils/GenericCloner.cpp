...
void GenericCloner::postFixUp(SILFunction *f) {
  ...
  struct AddressWalker final : public TransitiveAddressWalker<AddressWalker> {
    ...
    bool foundConsumingAddressUse = false;
  ...
  public:
    ...
    bool visitUse(Operand *use) {
      ...
      if (isa<DestroyAddrInst>(user)) {
        foundConsumingAddressUse = true;
      } else if (auto *copyAddr = dyn_cast<CopyAddrInst>(user)) {
        foundConsumingAddressUse = copyAddr->isTakeOfSrc();
      } else if (auto *explicitCopyAddr =
                     dyn_cast<ExplicitCopyAddrInst>(user)) {
        foundConsumingAddressUse = explicitCopyAddr->isTakeOfSrc();
      } else if (auto *load = dyn_cast<LoadInst>(user)) {
        foundConsumingAddressUse =
            (load->getOwnershipQualifier() == LoadOwnershipQualifier::Take);
      }
  ...
  for (auto *enumInst : enumInstsToFixup) {
    ...
    SSAPrunedLiveness liveness(enumInst->getFunction(), &discoveredBlocks);
    liveness.initializeDef(enumInst);
    AddressWalker walker(liveness);
    auto result = walker.walk(enumInst);
    if (result == AddressUseKind::PointerEscape ||
        result == AddressUseKind::Unknown) {
      continue;
    }
    if (walker.foundConsumingAddressUse) {
      PrunedLivenessBoundary boundary;
      liveness.computeBoundary(boundary);
      for (auto *lastUser : boundary.lastUsers) {
        SILBuilderWithScope::insertAfter(lastUser, [&](SILBuilder &builder) {
          builder.createDestroyAddr(lastUser->getLoc(), enumInst->getOperand());
        });
      }
    }
  }
