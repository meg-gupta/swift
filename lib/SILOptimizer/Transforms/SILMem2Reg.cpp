...
public:
  struct Owned {
    ...
    SILValue replacement(AllocStackInst *asi, SILInstruction *toReplace) {
      ...
      assert(move || asi->getType().isOrHasEnum());
      // If we had a trivial value stored in a lexical non trivial enum, we may
      // not have created a move, return stored value;
  ...
  struct Guaranteed {
    ...
    SILValue replacement(AllocStackInst *asi, SILInstruction *toReplace) {
      ...
      // For non trivial lexical enum stack locations, we may not have created
      // any borrow, if the stored value was trivial.
      assert(isGuaranteedLexicalValue(stored) || borrow ||
             asi->getType().isOrHasEnum());
