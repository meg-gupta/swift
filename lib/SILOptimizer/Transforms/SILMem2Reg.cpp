...
static StorageStateTracking<LiveValues>
beginNewLifetimeAfterStoreBorrow(AllocStackInst *asi, StoreBorrowInst *sbi) {
  CopyValueInst *cvi = nullptr;
  SILBuilderWithScope::insertAfter(sbi, [&](SILBuilder &builder) {
    SILLocation loc = RegularLocation::getAutoGeneratedLocation(sbi->getLoc());
    cvi = builder.createCopyValue(loc, sbi->getSrc());
  });
  StorageStateTracking<LiveValues> vals = {{sbi->getSrc(), SILValue(), cvi},
                                           /*isStorageValid=*/true};
  return vals;
}
...
class StackAllocationPromoter {
  ...
  /// alloc_stacks to which we can only store_borrow
...
SILInstruction *StackAllocationPromoter::promoteAllocationInBlock(
  ...
  for (auto bbi = blockPromotingWithin->begin(),
    ...
    if (auto *sbi = dyn_cast<StoreBorrowInst>(inst)) {
      ...
      // If we met a store_borrow before this one, delete it.
      if (lastStoreInst) {
      ...
      if (runningVals) {
        endNewLifetimeBeforeInst(asi, /*beforeInstruction=*/sbi, ctx,
                                 runningVals->value);
      }
      // The current store is now the lastStoreInst (until we see
      // another).
      lastStoreInst = {sbi, /*isStorageValid=*/true};
      runningVals = beginNewLifetimeAfterStoreBorrow(asi, sbi);
      continue;
    }
    ...
    if (auto *dsi = dyn_cast<DeallocStackInst>(inst)) {
      if (dsi->getOperand() == asi) {
        deallocPoints[dsi->getParentBlock()] = dsi;
        break;
      }
    }
  }
...
>>>>>>> 55d9b4f21de (Handle store_borrow in SILMem2Reg)
      SILValue borrow = SILValue();
      SILValue copy = SILValue();
      if (shouldAddLexicalLifetime(asi)) {
...
=======
  ...
  bool isStoreBorrowLocation = false;
>>>>>>> 55d9b4f21de (Handle store_borrow in SILMem2Reg)
  Optional<StorageStateTracking<LiveValues>> runningVals;
  ...
  for (auto bbi = parentBlock->begin(), bbe = parentBlock->end(); bbi != bbe;) {
    ...
    if (auto *sbi = dyn_cast<StoreBorrowInst>(inst)) {
      if (sbi->getDest() == asi) {
        auto oldRunningVals = runningVals;
        runningVals = {
            LiveValues::toReplace(asi, /*replacement=*/sbi->getSrc()),
            /*isStorageValid=*/true};
        if (oldRunningVals) {
          endNewLifetimeBeforeInst(asi, /*beforeInstruction=*/sbi, ctx,
                                   oldRunningVals->value);
