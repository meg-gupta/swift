...
static bool hasOnlyNoneOwnershipIncomingValues(SILArgument *phi) {
  ...
  for (unsigned arg1Idx = 0; arg1Idx < block->getNumArguments(); ++arg1Idx) {
    for (unsigned arg2Idx = arg1Idx + 1; arg2Idx < block->getNumArguments();) {
      ...
      if (valuesAreEqual(arg1, arg2)) {
        ...
        // None ownership. To replace, create a copy_value of the duplicate arg.
        if (arg1->getOwnershipKind() == OwnershipKind::Owned &&
            arg2->getOwnershipKind() == OwnershipKind::Owned) {
          assert(hasOnlyNoneOwnershipIncomingValues(arg1));
          assert(hasOnlyNoneOwnershipIncomingValues(arg2));
          SILBuilderWithScope builder(&block->front());
          auto copy = builder.createCopyValue(
              RegularLocation::getAutoGeneratedLocation(), arg1);
          arg2->replaceAllUsesWith(copy);
          erasePhiArgument(block, arg2Idx);
        } else if (arg1->getOwnershipKind() == OwnershipKind::Owned &&
                   arg2->getOwnershipKind() == OwnershipKind::None) {
          assert(hasOnlyNoneOwnershipIncomingValues(arg1));
          arg1->replaceAllUsesWith(arg2);
          erasePhiArgument(block, arg1Idx);
        } else if (arg1->getOwnershipKind() == OwnershipKind::None &&
                   arg2->getOwnershipKind() == OwnershipKind::Owned) {
          assert(hasOnlyNoneOwnershipIncomingValues(arg2));
