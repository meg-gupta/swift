  ...
  if (directResultType) {
    ...
    if (NeedsReturn) {
      if (fnConv.getNumDirectSILResults() > 1 &&
          (fnConv.hasGuaranteedAddressResults() ||
        ...
        if (fnConv.hasGuaranteedResults()) {
          epilogBB->createPhiArgument(resultType, OwnershipKind::Guaranteed);
        } else if (fnConv.hasGuaranteedAddressResults()) {
          // TODO: For epliog blocks with multiple predecessors, we will have
          // address phis which are banned. Fix this case.
          epilogBB->createPhiArgument(resultType, OwnershipKind::None);
        }
      } else {
