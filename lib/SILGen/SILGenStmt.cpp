...
void SILGenFunction::emitReturnExpr(SILLocation branchLoc,
  ...
  if (F.getConventions().hasIndirectSILResults()) {
    ...
    assert(loweredResultTy == loweredRetTy);
    RV = emitRValue(ret);
    std::move(RV)
        .ensurePlusOne(*this, CleanupLocation(ret))
        .forwardAll(*this, directResults);

    SmallVector<SILValue, 4> newResults;
    for (auto value : directResults) {
      auto g = B.createMarkUnresolvedGuaranteedValueInst(
          RegularLocation::getAutoGeneratedLocation(), value);
      newResults.push_back(g);
    }
    Cleanups.emitBranchAndCleanups(ReturnDest, branchLoc, newResults);
  }  
  else {
    // SILValue return.
    FullExpr scope(Cleanups, CleanupLocation(ret));
