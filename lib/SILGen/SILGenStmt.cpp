...
bool SILGenFunction::emitGuaranteedReturn(
  ...
  if (afd->getAttrs().hasAttribute<UnsafeSelfDependentResultAttr>()) {
    ...
    if (F.getConventions().hasGuaranteedResult() && isa<LoadExpr>(ret)) {
      auto rv = emitLoadOfLValue(RegularLocation::getAutoGeneratedLocation(),
                                 std::move(lvalue),
                                 SGFContext::AllowGuaranteedPlusZero,
                                 /*isBaseGuaranteed*/ true);
      SILValue result = std::move(rv).getAsSingleValue(*this, loc).getValue();
      assert(!result->getType().isTrivial(getFunction()));
      ...
      // TODO: Introduce a new SIL instruction to allow returning borrowed
      // values from addresses that are guaranteed to be valid in the caller.
      result = B.createUncheckedOwnershipConversion(
          RegularLocation::getAutoGeneratedLocation(), result,
    ...
    auto resultValue = emitBorrowedLValue(
        RegularLocation::getAutoGeneratedLocation(), std::move(lvalue));
