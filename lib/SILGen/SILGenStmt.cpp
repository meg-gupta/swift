...
void SILGenFunction::emitReturnExpr(SILLocation branchLoc,
  ...
  if (F.getConventions().hasIndirectSILResults()) {
    ...
    if (isa<LiteralExpr>(ret)) {
      ...
      // For ~Copyable types, introduce an unchecked_ownership_conversion to
      // avoid introducing a use at return outside the scope of proactively
      // added mark_unresolved_noncopyable_value instruction.
      SILType selfType = F.getConventions().getSILArgumentType(
      F.getConventions().getNumSILArguments() - 1, F.getTypeExpansionContext());
      selfType = F.mapTypeIntoContext(selfType);
      if (selfType.isMoveOnly() && F.getConventions().hasGuaranteedResults()) {
