...
/// borrowed, and forwareded before being used as the base operand of
  ...
  mutating func walkDown(operand: Operand) -> WalkResult {
    ...
    case let borrow as BeginBorrowInst:
      ...
      for endOperand in BeginBorrowValue(borrow)!.scopeEndingOperands {
        switch endOperand.instruction {
        case is EndBorrowInst:
          if leafUse(endOperand) == .abortWalk {
            return .abortWalk
          }
        case is BranchInst:
          let phi = Phi(using: endOperand)!
          if walkDownUses(of: phi.value, using: endOperand) == .abortWalk {
            return .abortWalk
          }
        default:
