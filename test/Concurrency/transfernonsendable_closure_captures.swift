...
func testCopyableNonsendableStructWithEscapingMainActorAsync() {
  ...
  let _ = {
    escapingAsyncUse { @MainActor in
      useValue(x) // expected-error {{sending 'x' risks causing data races}}
      // expected-note @-1 {{task-isolated 'x' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
    }
  }
}
    ...
    let inner = {
      escapingAsyncUse { @MainActor in
        if let obj = obj { // expected-error {{sending 'obj' risks causing data races}}
                  // expected-note @-1 {{task-isolated 'obj' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
          useValue(obj)
        }
      }
    }
  ...
  let _ = { [weak obj1, weak obj2, weak obj3] in
    escapingAsyncUse { @MainActor in
      if let o1 = obj1 { // expected-error {{sending 'obj1' risks causing data races}}
        useValue(o1) // expected-note @-1 {{task-isolated 'obj1' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
      }
      if let o2 = obj2 { // expected-error {{sending 'obj2' risks causing data races}}
        // expected-note @-1 {{task-isolated 'obj2' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
        useValue(o2)
      }
      if let o3 = obj3 { // expected-error {{sending 'obj3' risks causing data races}}
        // expected-note @-1 {{task-isolated 'obj3' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
        useValue(o3)
      }
    }
  }
  ...
  func setupCallback() {
    let _ = { [weak self] in
      escapingAsyncUse { @MainActor in
        ...
        // expected-note @-1 {{task-isolated 'self' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
        useValue(self)
      }
    }
  }
    ...
    let _ = { [weak outer, weak inner] in
      escapingAsyncUse { @MainActor in
        if let o = outer { // expected-error {{sending 'outer' risks causing data races}}
          // expected-note @-1 {{task-isolated 'outer' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
          useValue(o)
        }
        if let i = inner { // expected-error {{sending 'inner' risks causing data races}}
          // expected-note @-1 {{task-isolated 'inner' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
          useValue(i)
        }
      }
    }
...
func testMultipleProtocolWeakCapturesNonsendable<T: ProtocolNonsendable, U: ProtocolNonsendable, V: ProtocolNonsendable>(_ obj1: T, _ obj2: U, _ obj3: V) {
  let _ = { [weak obj1, weak obj2, weak obj3] in
    escapingAsyncUse { @MainActor in
      if let o1 = obj1 { // expected-error {{sending 'obj1' risks causing data races}}
        // expected-note @-1 {{task-isolated 'obj1' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
        useValue(o1)
      }
      if let o2 = obj2 { // expected-error {{sending 'obj2' risks causing data races}}
        // expected-note @-1 {{task-isolated 'obj2' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
  ...
  let _ = {
    let _ = {
      escapingAsyncUse { @MainActor in
        ...
        // expected-note @-1 {{task-isolated 'nonsendable' is captured by a main actor-isolated closure. main actor-isolated uses in closure may race against later @concurrent uses}}
      }
    }
  }
}
